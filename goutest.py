...
class GPUDiagnostic:
    ...
    def check_cuda_installation(self):
        ...
        if code == 0:
            ...
            logger.info(f"NVCC: {stdout.split('\n')[0]}")
        else:
    ...
    def generate_report(self):
        ...
        # Resumo geral
        cuda_available = self.results.get('pytorch_gpu', {}).get('cuda_available', False)
            ...
            for model_size in ['s', 'm', 'l', 'x']:
                ...
                if model_key in self.results:
                    ...
                    if 'inference_time' in info:
                        logger.info(f"  YOLOv8-{model_size.upper()}: {info['inference_time']:.3f}s")
        ...
        if not cuda_available:
            ...
            logger.info("👉 pip install torch==2.2.1 torchvision==0.17.1 torchaudio==2.2.1 --index-url https://download.pytorch.org/whl/cu121")
        else:
