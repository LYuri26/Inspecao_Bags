...
class DiagnosticCompleto:
    ...
    def check_cuda_installation(self):
        ...
        if "release" in nvcc_output.lower():
            ...
            logger.info(f"NVCC: {nvcc_output.split('\n')[0]}")
        else:
    ...
    def check_pytorch_gpu(self):
        ...
        self.add_to_report("PyTorch", f"CUDA disponível: {torch_info['cuda_available']}")
        
        logger.info(f"PyTorch Version: {torch_info['version']}")
        logger.info(f"CUDA Available: {torch_info['cuda_available']}")
        
        if torch_info['cuda_available']:
            self.add_to_report("PyTorch", f"Versão CUDA (PyTorch): {torch_info['cuda_version']}")
            self.add_to_report("PyTorch", f"Versão cuDNN: {torch_info['cudnn_version']}")
    ...
    def test_gpu_performance(self):
        ...
        if not torch.cuda.is_available():
            self.add_to_report("Performance", "GPU não disponível para teste de performance")
    ...
    def test_yolov8_model(self, model_size='s'):
        ...
        try:
            ...
            if len(results) > 0 and hasattr(results[0], 'boxes'):
                model_info["detections"] = len(results[0].boxes) if results[0].boxes else 0
    ...
    def generate_recommendations(self):
        ...
        if "cpu" in torch.__version__.lower():
            ...
            recommendations.append("   pip install torch==2.2.1 torchvision==0.17.1 torchaudio==2.2.1 --index-url https://download.pytorch.org/whl/cu121")
        elif not torch.cuda.is_available():
            recommendations.append("⚠️ PyTorch não está detectando a GPU.")
            recommendations.append("👉 Verifique se os drivers NVIDIA estão instalados corretamente")
            recommendations.append("👉 Verifique se a GPU está funcionando no sistema")
        else:
            recommendations.append("✅ PyTorch está configurado corretamente com suporte GPU")
            ...
            if any(f'yolov8_{m}' in self.results for m in ['s', 'm', 'l', 'x']):
                ...
                if times:
                    ...
                    recommendations.append(f"   🚀 Mais rápido: YOLOv8-{fastest_model.upper()} ({fastest_time:.3f}s)")
                    
                    # Recomendação baseada na velocidade
                    if fastest_time < 0.1:
                        recommendations.append("   💡 Recomendado: YOLOv8-l ou YOLOv8-x para máxima precisão")
                    elif fastest_time < 0.2:
                        recommendations.append("   💡 Recomendado: YOLOv8-m para bom equilíbrio velocidade-precisão")
                    else:
                        recommendations.append("   💡 Recomendado: YOLOv8-s para máxima velocidade")
    ...
    def run_full_diagnostic(self):
        ...
        try:
            ...
            if torch.cuda.is_available():
                ...
                self.add_to_report("Performance", "⚠️ Testes de performance skipped - CUDA não disponível")
